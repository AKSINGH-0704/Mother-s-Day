...
const state = {
  ...
  finalMessage: '',
  audioData: null,      // base64 of uploaded audio
  audioFileName: '',
  audioUrl: null,       // blob URL after cloud upload
  shareData: null,
  poppedCount: 0,
  totalBalloons: 0,
...
function buildReverb(ctx) {
  const conv=ctx.createConvolver(), len=ctx.sampleRate*2;
...
// ─────────────────────────────────────────────
// URL LINK UTILITIES (LZ-String fallback)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// CREATOR: MEMORY CARDS + AUDIO UPLOAD
// ─────────────────────────────────────────────
...
function initBalloonsScreen() {
  ...
  if (audioInput) {
    ...
    if (state.audioFileName) {
      ...
      audioName.textContent = '🎵 ' + state.audioFileName;
    }
...
function initDeployScreen() {
  ...
  // Build shareData from current state (sync — so preview button always works)
  state.shareData = {
...
async function generateShareLink(linkInput) {
  // STEP 1: Try cloud upload + KV save
  let cloudLink = null;
  try {
    // Upload images
    ...
    if (state.audioData && !state.audioUrl) {
      ...
      } catch(e) { /* continue without audio in cloud */ }
    }
    ...
    if (resp.ok) {
      ...
      cloudLink = `${location.origin}${location.pathname}?s=${id}`;
    }
  } catch(e) { /* cloud unavailable */ }

  // STEP 2: If cloud failed, build inline LZ-String link (always works)
  if (cloudLink) {
    linkInput.value = cloudLink;
    wireShareButtons(cloudLink);
  } else {
    linkInput.value = 'Building your link…';
    try {
      // For inline, audio base64 is too large — include only if blob URL exists
      const inlinePayload = {
        relationship: state.shareData.relationship,
        name: state.shareData.name,
        finalMessage: state.shareData.finalMessage,
        audioUrl: state.audioUrl || null,
        balloons: state.shareData.balloons // may still have base64 images — that's fine
      };
      const inlineLink = buildInlineLink(inlinePayload);
      linkInput.value = inlineLink;
      wireShareButtons(inlineLink);
    } catch(e) {
      linkInput.value = 'Error building link. Please try again.';
    }
  }
}
...
async function checkUrl() {
  ...
  if (id || inlineParam) {
    ...
    if (inlineParam) {
      // Decode from URL directly — no backend needed
...
async function beginRecipientFlow(data) {
  ...
  // Preload personal audio if URL provided (and not already set by preview)
...
function showFinalReveal() {
  ...
  // Personal audio already looping — let it continue
